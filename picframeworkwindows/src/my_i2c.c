...
int i2c_master_handler()
{
    ...
    if (!overrun_error)
    {
        ...
        {
            //if SSPCON2bits.ACKSTAT == 0 we receive ACK, if 1 we receive nack
            case I2C_IDLE:
            {
                IdleI2C();
                 break;
            };
            case I2C_I2C_MASTER_SEND_ADDRESS_FOR_SEND_DATA:
            {
                ic_ptr->status = I2C_MASTER_SEND_DATA;
                //char tempdata = SSPBUF;
            ...
            case I2C_MASTER_SEND_DATA:
            {
                ic_ptr->event_count++;
                // send mode
                if (ic_ptr->outbufind < ic_ptr->outbuflen && !SSPCON2bits.ACKSTAT)
                {
                    SSPBUF = ic_ptr->outbuffer[ic_ptr->outbufind];
                    //WriteI2C(ic_ptr->outbuffer[ic_ptr->outbufind]);
            ...
            case I2C_MASTER_SEND_ADDRESS_FOR_RECEIVE_DATA:
            {
            ...
            {
                if (!SSPCON2bits.ACKSTAT)  //we get an ack
                {
                    SSPCON2bits.RCEN = 1;
                ...
                else  //no response to our read request
                {
            ...
            case I2C_MASTER_DATA_READ:
            {
                if (data_read )
                {
                    ic_ptr->buffer[ic_ptr->bufind] = i2c_data;
                    ...
                    else
                    {
                        ...
                        IdleI2C();
                        if(ic_ptr->buflen == 7)
                        ToMainLow_sendmsg(7, MSGT_I2C_DATA, ic_ptr->buffer);
                        else if(ic_ptr->buflen == 2)
                        ToMainLow_sendmsg(2, MSGT_I2C_DATA, ic_ptr->buffer);
                        ic_ptr->status = I2C_STOP_IDLE;
                        ic_ptr->bufind = 0;
    ...
    /*if(SSPSTATbits.I2C_START)
    {
      //  WriteI2C(ic_ptr->slave_addr); //if w

       WriteI2C(ic_ptr->slave_addr | 01); //if read

       IdleI2C();
       if(!SSPCON2bits.ACKSTAT) //ack
      {
IdleI2C();
        WriteI2C(test);
        StopI2C();
...
unsigned char i2c_master_send(unsigned char length, unsigned char *msg, unsigned char slave_addr) {
    ...
    ic_ptr->outbufind = 0;
    ic_ptr->status = I2C_I2C_MASTER_SEND_ADDRESS_FOR_SEND_DATA;
    ic_ptr->slave_addr = slave_addr;
...
unsigned char i2c_master_recv(unsigned char length, unsigned char slave_addr) {
    ...
    ic_ptr->status = I2C_MASTER_SEND_ADDRESS_FOR_RECEIVE_DATA;
    ic_ptr->slave_addr = slave_addr;
